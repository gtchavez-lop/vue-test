...
const actions = {
  addgsinfo({ commit }, formData) {
    axios
      .post('/api/gradesheetinfo', formData)
      .then((response) => {
  ...
  displaygstable({ commit, state }) {
    axios
      .get('/api/displaygrade/' + state.route)
      .then((response) => {
        //   this.currentUser = response.data
  ...
  addStudGradesheet({ commit }, addStud) {
    ...
    return axios
      .post('api/gradesheetstudent/', addStud)
      .then((response) => {
        //   this.currentUser = response.data
        commit('setLoading', false);
        // console.log('adding successful' , response.data);
  ...
  cardwithpage({ commit }, route, page) {
    axios
      .get(`/api/paginatecard/` + route + `?page=${page}`)
      .then(({ data }) => {
        this.cards = data;
        // console.log(data);
      })
