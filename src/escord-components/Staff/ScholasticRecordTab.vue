      ...
      <div class="section profile-content">
        ...
        <div>
          <div class="md-layout escord md-gutter md-alignment-center-center">
            <div
              class="md-layout-item md-size-33 md-layout md-gutter md-alignment-center-center"
            >
              <md-button @click="showevaluationtable"
                >EVALUATION FORM</md-button
              >
            </div>
            <div
              class="md-layout-item md-size-33 md-layout md-gutter md-alignment-center-center"
            >
              <md-button @click="showscholastictable"
                >SCHOLASTIC RECORD</md-button
              >
            </div>
  ...
  <md-button @click="showscholastictable">SCHOLASTIC RECORD</md-button>-->
            ...
            <md-table v-model="users.data" md-sort="name" md-sort-order="asc">
              <md-table-row class="sec">
                ...
                <md-table-head class="text-center"
                  >Year and Section</md-table-head
                >
              ...
              <md-table-row v-for="(_, index) in users.data" :key="index">
                ...
                <md-table-cell class="text-center">
                  ...
                  {{ users.data[index].middlename }}
                  {{ users.data[index].surname }}
                </md-table-cell>
          ...
          <div v-else>
            ...
            <md-table v-model="eval.data" md-sort="name" md-sort-order="asc">
              <md-table-row class="sec">
                <md-table-head class="text-center"
                  >Evaluation Number</md-table-head
                >
                <md-table-head class="text-center"
                  >Student Number</md-table-head
                >
                <md-table-head class="text-center">Student Name</md-table-head>
                <md-table-head class="text-center">Program</md-table-head>
                <md-table-head class="text-center"
              ...
              <md-table-row v-for="(_, index) in eval.data" :key="index">
                ...
                <md-table-cell class="text-center">
                  {{ eval.data[index].firstname }}
                  {{ eval.data[index].middlename }}
                  {{ eval.data[index].surname }}
                </md-table-cell>
              ...
              <pagination
                no-arrows
                :page-count="eval.last_page"
                :value="eval.current_page"
                :total="eval.total"
                @input="scholmis"
              />
            </md-table>
...
export default {
  ...
  methods: {
    ...
    async scholmis(scholpage = 1) {
      await axios
        .get('/api/showscolmis/?page=' + scholpage + '&search=' + this.search)
        .then(({ data }) => {
          this.users = data;
    ...
    async evalmis(page = 1) {
      await axios
        .get('/api/evalform/?page=' + page + '&search=' + this.search)
        .then(({ data }) => {
          this.eval = data;
          //     this.currentpage = page
        });
