          ...
          >
            <div class="__gradesheet-subject md-layout-item md-size-100">
              <h5 class="md-subheading">
                SEMESTER : {{ topviewdata.semester }}SY:
                {{ topviewdata.sem_startyear }} - {{ topviewdata.sem_endyeaer }}
              </h5>

              <h4>
                <strong
                  >STUDENT NUMBER : {{ topviewdata.student_number }}
                </strong>
            ...
            >
              ...
              <p class="md-caption">COURSE : {{ topviewdata.course }}</p>

              <p class="md-caption">YEAR : {{ topviewdata.section }}</p>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    archievebtn() {
      axios
        .put('api/archievegs/' + this.$route.params.gradeshid, {
          status_archieve: '1',
        })
        .then((response) => {
